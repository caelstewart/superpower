  ...
  const server = new McpServer(
    { name: "superpower", version: version() },
    {
      instructions:
        ...
        "METHOD/PROCESS FILES (only if one comes up — most users won't have one, don't ask): " +
        "if the user brings their own writing process, framework, or copy guide, fold its " +
        "concrete structure into the BRIEF you pass to generate_copy (the specific headline " +
        "formula, hooks, sections, CTA rules it calls for) — that brief is the only thing that " +
        "carries the method into generation, so make it explicit, not a vague summary. The " +
        "method decides WHAT the copy says; the captured voice decides HOW it sounds. Never " +
        "pass the method as generation instructions or let its style opinions override the " +
        "voice's examples.",
    }
  );
